...
- [ ] T2.1 JobState：step/status/total/done/skipped/current_item/logs（deque 上限 200）/error + threading.Lock
- [ ] T2.2 prepare 执行函数：复用 prepare_tasks 逻辑，注入当前难度权重，写 tasks.json，返回分配摘要
...
- [ ] T4.1 curl 链路：prepare → status → validate（不调模型部分）
- [ ] T4.2 curl 链路：generate 启动 / 进度轮询 / 停止 / 断点续跑（小批量 1~2 条真实调用）
...
- [ ] T5.1 Vite + React 脚手架；依赖：antd、react-router-dom、axios、dayjs、@ant-design/plots（按 T5.0 确认的风格实现）
- [ ] T5.2 vite.config.js：proxy `/api` 与 `/videos` → http://localhost:8000
- [ ] T5.3 api.js：axios 实例 + 全部接口封装 + 统一错误 message
- [ ] T5.4 App 布局：左侧菜单 + 顶部流水线状态条 + react-router

**✅ 阶段 5 验证清单**
- [ ] T5.0 原型经用户确认风格后再动 T5.1
...
- [ ] T6.1 Dashboard（F1）：Stepper + 每步执行按钮 + 进度条/当前条目/滚动日志（running 时 1s 轮询）+ 停止按钮 + 统计卡片与分布图
...
- [ ] T7.1 `npm run build` → FastAPI 托管 dist，单命令 `python server/main.py` 端到端验证
- [ ] T7.2 端到端走查 spec.md 第 8 节验收标准 1~5，小批量真实生成一轮
- [ ] T7.3 README 增加「Web 界面」章节
- [ ] T7.4 修复联调问题，更新 DEVLOG.md
