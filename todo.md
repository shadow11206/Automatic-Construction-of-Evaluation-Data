...
- [x] T10.1 类目配置页「从 Excel 导入」：上传 xlsx/xls/csv 解析预览（追加/覆盖两种模式），确认后由「保存配置」统一写回
- [x] T10.2 视频管理页批量删除：表格勾选列 + 「删除选中（N）」按钮，批量删除接口逐个返回成功/失败明细

**✅ 验证清单**
- [x] 批量删除：2 个存在视频删除成功 + 不存在文件返回 failed 明细
- [x] 导入解析：xlsx（2行15条）与 csv（8行80条）均正确识别列名；坏文件 400 友好提示
- [x] 浏览器确认：类目页有「从 Excel 导入」按钮；视频表格有勾选列

## 增量需求（2026-07-30 第四批）

- [x] T11.1 视频库交互重构：删除头部「全选/清空」按钮；「参与」列与勾选列合并（一套勾选=视频配置）；批量删除按钮移入模块头部；新增「已使用/未使用」筛选；「保存清单」改名「保存视频配置」；新增「清空视频配置」（后端清单支持保存为空）
- [x] T11.2 事故恢复：验证清空接口时误清用户配置，已恢复为全选 5 个并告知用户

**✅ 验证清单**
- [x] 空清单保存/读回正常（save_video_list 允许空）
- [x] 浏览器：五筛选按钮、三操作按钮、勾选列头「配置」、无「参与」列、默认勾选已配置视频、未使用筛选生效
